...
hairpinHelper.py was created to address an issue with some datasets where there are multiple miRNAs in the mature.fa file that correspond to a single hairpin sequence in the hairpin.fa file. This would occur in instance where multiple miRNAs come from a single locus. miRScore requires that each hairpin have only one corresponding miRNA. This is because the way miRScore handles MIRNA names in the programming. To help users solve this issue, we've included this script to quickly resolve the instance where two or more miRNAs (i.e. osa-miR159a.1 and osa-miR159a.2) has only a single locus present in the hairpin file (osa-miR159a).
...
## Usage
```
python hairpinHelper.py [-help] -mature MATUREFILE.fa -hairpin HAIRPINFILE.fa
```
## Options
