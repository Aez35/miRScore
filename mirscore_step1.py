...
for bam in bamfiles:
    ...
    #If not present, add to dictionary of failed mirna candidates {failed}.
        if reads.count(x.seq) > 0:
            ...
            if mirnas.get(x.name)== None and failed.get(x.name) == None:
            #If not in {mirnas}, add it.
                print(x.name + " not yet in dictionary")
                mirnas[x.name]=reads.count(x.seq)

            elif mirnas.get(x.name) != None and failed.get(x.name) == None:
            #If mirna already exists in {mirnas}, add reads to existing entry.
                print(x.name + " in dictionary, appending read count")
                mirnas[x.name]=reads.count(x.seq) + mirnas[x.name]
                print(mirnas[x.name])

            elif mirnas.get(x.name) == None and failed.get(x.name) != None:
                print(x.name + " already in failed")
        else:
        #If no reads detected, add to {failed}.
            if failed.get(x.name) == None:
                failed[x.name]='No reads'
                print(x.name + " not present in " + Path(bam).stem + "... Attempting to add to failed")
            else:
                print(x.name + " no reads present but already in failed.")
            ...
            #Check if miRNA was not detected in at least one of the bam files. If so, remove it from {mirna}.
                print(x.name + " found in mirnas, removing now...")
                mirnas.pop(x.name)
            elif failed.get(x.name) != None and mirnas.get(x.name) == None:
                print(x.name + " found in failed, but not mirna")
    reads.clear()

print(str(len(mirnas)) + " potential miRNAs found.")
print(str(len(failed)) + " miRNAs failed to have reads in all files.")